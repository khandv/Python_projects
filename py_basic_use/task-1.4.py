...
foo
"""
# Словарь пространств - {"имя": ["родитель", ["переменные]]}
scopes = {'global': [None, []]}

def add(scope, var):
    if scope in scopes:
        scopes[scope][1].append(var)

def create(scope, parent):
    if  scope not in scopes:
        scopes[scope] = ['', []]
        scopes[scope][0] = parent

def get(scope, var):
    if scope in scopes:
        parent = scopes[scope][0]
        if var in scopes[scope][1]:
            print(scope)
        elif parent == None:
            print(None)
        else: get(parent, var)
