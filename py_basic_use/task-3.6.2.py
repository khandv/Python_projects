...
def get_token(url, data):
    result_token = requests.post(url, data)
    json_data = json.loads(result_token.text)
    return json_data["token"]


headers = {"X-Xapp-Token" : get_token(url, data)}
artist_dict = {}
with open("dataset_24476_4.txt", "r") as artists:
    for artist in artists:
        result = requests.get(f'https://api.artsy.net/api/artists/{artist.strip()}', headers=headers)
        result.encoding = 'utf-8'
        if result.status_code == 200:
            json_result = json.loads(result.text)
            artist_dict[json_result['sortable_name']] = int(json_result['birthday'])

sorted_artists = sorted(artist_dict.items(), key=lambda x: (x[1], x[0]))

with open('output_24476_4.txt', 'a', encoding='utf-8') as f:
    for key, item in sorted_artists:
        print(key)
        f.write(f'{key}\n')
